...
public class Picture implements Comparable<Picture> {
    ...
    public static void main(String[] args) {
        Picture pic;
        try {
            pic = new Picture("[1997-001][camin][001][Caminul 15].jpg", "f:\\Personal\\Picture",
                    "1997\\1997-001 Camin");
            pic.ShowInJPanel();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public String toACDSee() {
        ...
        returnStr = returnStr + "<Folder>&lt;Local\\"
        ...
        for (int i = 0; i < tmp.size(); i++) {
            ...
            returnStr = returnStr + "<AssetCategory>" + ((Category) tmp.get(i)).getFullName()
                    + "</AssetCategory>";
            if (i == tmp.size() - 1) {
                returnStr = returnStr + "</AssetCategoryList>\r\n";
            }
    ...
    public void show() {
        System.out.print(this.getRoot() + File.separator + this.getPath() + File.separator
                + this.getName() + " - ");
        Vector tmp = this.getKeywords();
        for (int i = 0; i < tmp.size(); i++) {
    ...
    public String toXML(String indent) {
        ...
        rez = rez + indent + "\t<" + Picture.TAG_PATH + ">" + this.getPath() + "</"
                + Picture.TAG_PATH + ">\r\n";
        rez = rez + indent + "\t<" + Picture.TAG_NAME + ">" + this.getName() + "</"
                + Picture.TAG_NAME + ">\r\n";
        if (this.getRating() != 0) {
            rez = rez + indent + "\t<" + Picture.TAG_RATING + ">" + this.getRating() + "</"
                    + Picture.TAG_RATING + ">\r\n";
        }
        if (this.address != null) {
            rez = rez + indent + "\t<" + Picture.TAG_ADDRESS + ">" + this.address.getId() + "</"
                    + Picture.TAG_ADDRESS + ">\r\n";
        }
        ...
        if (total > 0) {
            ...
            for (int i = 0; i < total; i++) {
                rez = rez + indent + "\t\t<" + Picture.TAG_CATEGORY + ">"
                        + this.getKeywords().get(i).getId() + "</" + Picture.TAG_CATEGORY + ">\r\n";
            }
            rez = rez + indent + "\t</" + Picture.TAG_CATEGORYLIST + ">" + "\r\n";
        }
    ...
    private void processACDSeeNode(Node xmlPic, String root) {
        /*
         * NodeList children = xmlPic.getChildNodes(); String folder = ""; String fileType = "";
         * String name = ""; String rating = "0"; if (children != null) { int len =
         * children.getLength(); // System.out.println("childs len: " + len); Node tmp; for (int i =
         * 0; i < len; i++) { tmp = children.item(i); if (tmp.getNodeType() == Node.ELEMENT_NODE) {
         * // System.out.print(tmp.getNodeName() + "-"); if
         ...
         * tmp.getChildNodes().item(0).getNodeValue(); } else if
         * (tmp.getNodeName().equalsIgnoreCase( "AssetCategoryList")) { NodeList catList =
         * tmp.getChildNodes(); // System.out.print(tmp.getNodeName() + "-"); int len1 =
         * catList.getLength(); Node tmp1; for (int j = 0; j < len1; j++) { tmp1 = catList.item(j);
         * if (tmp1.getNodeType() == Node.ELEMENT_NODE) { // System.out.print(tmp.getNodeName() +
         * "-"); if (tmp1.getNodeName().equalsIgnoreCase( "AssetCategory")) {
         * this.addKeyword(tmp1.getChildNodes() .item(0).getNodeValue()); } } } } } } if (fileType
         * != null && fileType.equalsIgnoreCase("JPEG")) { this.setName(name); this.setRoot(root);
         * folder = folder.substring(folder.indexOf("\\") + 1); folder =
         * folder.substring(folder.indexOf("\\") + 1, folder .length() - 1); if
         * (folder.indexOf(root.substring(3)) != 0) { this.name = null; } else { this.path =
         * folder.substring(root.length() - 2); } Integer rat = new Integer(rating);
         * this.setRating(rat.intValue()); } else { this.setName(null); } } else {
         * this.setName(null); }
         */
    }
                        ...
                        for (int j = 0; j < len1; j++) {
                            ...
                            if (tmp1.getNodeType() == Node.ELEMENT_NODE) {
                                ...
                                if (tmp1.getNodeName().equalsIgnoreCase("Category")) {
                                    Integer itmp = new Integer(tmp1.getChildNodes().item(0)
                                            .getNodeValue());
                                    /*
                                     * Address add = address.getAddressById(itmp.intValue()); if
                                     * (add != null) { this.address = add; } else {
                                     */
                                    logger.debug("size of category: "
     ...
     * Picture.TAG_HEAD); ElementImpl tmp = null; tmp = new ElementImpl(doc, Picture.TAG_NAME);
     * tmp.appendChild(new TextImpl(doc, this.getName())); node.appendChild(tmp); tmp = new
     * ElementImpl(doc, Picture.TAG_PATH); tmp.appendChild(new TextImpl(doc, this.getPath()));
     * node.appendChild(tmp); if (this.getAddress() != null) { tmp = new ElementImpl(doc,
     * Picture.TAG_ADDRESS); tmp.appendChild(new TextImpl(doc, "" + this.getAddress().getId()));
     * node.appendChild(tmp); } if (this.getRating() != 0) { tmp = new ElementImpl(doc,
     * Picture.TAG_RATING); tmp.appendChild(new TextImpl(doc, "" + this.getRating()));
     * node.appendChild(tmp); } Vector<Category> add = this.getKeywords(); if (add.size() > 0) { tmp
     * = new ElementImpl(doc, Picture.TAG_CATEGORYLIST); ElementImpl tmp2 = null; for (int i = 0; i
     * < add.size(); i++) { tmp2 = new ElementImpl(doc, Picture.TAG_CATEGORY); tmp2.appendChild(new
     * TextImpl(doc, "" + add.get(i).getId())); tmp.appendChild(tmp2); } node.appendChild(tmp); }
     * return node; }
     */
